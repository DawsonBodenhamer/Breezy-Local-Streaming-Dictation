...
def _windows_virtual_key(key: str) -> int:
    """Resolve the narrow key set supported by the native Windows backend."""
    if key == "f24":
        return 0x87  # VK_F24
...
class HotkeyListener:
    ...
    def __init__(
        ...
        self._windows_ready = threading.Event()
        self._windows_error: str | None = None
    ...
    def start(self) -> None:
        ...
        if sys.platform == "win32" and self.mode == "toggle":
    ...
    def stop(self) -> None:
        ...
        if self._listener is not None:
            ...
            self._listener = None
        if self._windows_thread_id is not None:
            import ctypes

            ctypes.windll.user32.PostThreadMessageW(
                self._windows_thread_id, 0x0012, 0, 0  # WM_QUIT
            )
    ...
    def _start_windows_hotkey(self) -> None:
        """Register a toggle chord through the native Windows message queue."""
        self._windows_ready.clear()
        self._windows_error = None
        self._thread = threading.Thread(
            target=self._windows_hotkey_loop,
            daemon=True,
            name="dictation-windows-hotkey",
        )
        self._thread.start()
        if not self._windows_ready.wait(timeout=2.0):
            raise RuntimeError(f"Timed out registering Windows hotkey {self.binding}")
        if self._windows_error is not None:
            raise RuntimeError(self._windows_error)
        log.info(
            "Hotkey listener started (RegisterHotKey): %s [%s mode]",
            self.binding,
            self.mode,
        )
    ...
    def _windows_hotkey_loop(self) -> None:
        ...
        virtual_key = _windows_virtual_key(self._key)
        hotkey_id = 1
        ...
        if not user32.RegisterHotKey(None, hotkey_id, modifiers, virtual_key):
            self._windows_error = (
                f"Windows refused hotkey {self.binding}; it may be in use"
            )
            self._windows_ready.set()
            return
        ...
        try:
            while not self._stop_event.is_set():
                ...
                if message.message == 0x0312:  # WM_HOTKEY
                    # RegisterHotKey + MOD_NOREPEAT already guarantees one
                    # message per physical press.
                    with self._lock:
                        self._key_released = True
                    self._handle_press()
        finally:
